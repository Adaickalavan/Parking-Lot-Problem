...
type Carpark struct {
	...
	EmptySlot   util.PriorityQueue //Heap containing sorted empty slots in ascending order
...
func (carpark *Carpark) removeCar(slotNo int) {
	...
	heap.Push(&carpark.EmptySlot, &util.Item{Value: slotNo})
}
